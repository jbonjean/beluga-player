 ...
 * Label skin.
 */
public class BelugaLabelSkin extends ComponentSkin implements LabelListener
{
	private Font font;
	private Color color;
	...
	public void layout()
	{
		...
		hasTextKey = label.getTextKey() != null && !label.getTextKey().isEmpty();
		if (hasTextKey)
			textKey = label.getTextKey() + ": ";
		...
		if (text != null)
		{
			...
			if (n > 0)
			{
				...
				// add the text key in first position
				Rectangle2D textKeyTextBounds = null;
				if (hasTextKey)
				{
					GlyphVector glyphVector = font.deriveFont(Font.BOLD).createGlyphVector(fontRenderContext, textKey);
					glyphVectors.add(glyphVector);
					textKeyTextBounds = glyphVector.getLogicalBounds();
					textHeight += textKeyTextBounds.getHeight();
				}
				...
				if (wrapText)
				{
					...
					int i = 0;
					int start = 0;
					float lineWidth = textKeyTextBounds == null ? 0 : (float) textKeyTextBounds.getWidth();
					int lastWhitespaceIndex = -1;
	...
	public void paint(Graphics2D graphics)
	{
		...
		if (glyphVectors != null && glyphVectors.getLength() > 0)
		{
			...
			for (int i = 0, n = glyphVectors.getLength(); i < n; i++)
			{
				...
				// first entry after text key, display on the same line, just add an indent
				// only tested with align left, would probably fail with others
				if (hasTextKey && i == 1)
					x += glyphVectors.get(0).getLogicalBounds().getWidth();
				...
				// inline text key, we don't go to next line
				if (!hasTextKey || (hasTextKey && i > 0))
	 ...
	 * 
	 * @param font
	 *            A {@link ComponentSkin#decodeFont(String) font specification}
	 */
	 ...
	 * 
	 * @param font
	 *            A dictionary {@link Theme#deriveFont describing a font}
	 */
	 ...
	 * 
	 * @param color
	 *            Any of the {@linkplain GraphicsUtilities#decodeColor color values recognized by Pivot}.
	 */
	...
	/**
	 * Sets whether the text of the label will be wrapped to fit the Label's width. Note that for wrapping to occur, the Label must specify a
